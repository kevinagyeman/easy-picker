...
export class WheelPicker {
	...
	private getColumnConfigs(): Array<{
		values: (string | number)[];
		...
		if (format === "datetime" || format === "time") {
			...
			configs.push({
				values: hours,
				selected: String(currentHour).padStart(2, "0"),
			});
			...
			configs.push({
				values: minutes,
				selected: String(currentMinute).padStart(2, "0"),
			});
		}
	...
	private updateColumnTransform(columnIndex: number): void {
		...
		items.forEach((item, index) => {
			...
			const centerOffset =
				offset - (this.itemHeight * (this.visibleItems - 1)) / 2;
	...
	private updateColumns(): void {
		...
		if (format === "date" || format === "datetime") {
			...
			const year = this.columns[2].values[
				this.columns[2].selectedIndex
			] as number;
	...
	private onDateChange(): void {
		...
		if (format === "date" || format === "datetime") {
			const monthIndex = this.columns[0].selectedIndex;
			const day = this.columns[1].values[
				this.columns[1].selectedIndex
			] as number;
			const year = this.columns[2].values[
				this.columns[2].selectedIndex
			] as number;
			...
			if (format === "datetime") {
				const hour = parseInt(
					this.columns[3].values[this.columns[3].selectedIndex] as string,
				);
				const minute = parseInt(
					this.columns[4].values[this.columns[4].selectedIndex] as string,
				);
			...
			const hour = parseInt(
				this.columns[0].values[this.columns[0].selectedIndex] as string,
			);
			const minute = parseInt(
				this.columns[1].values[this.columns[1].selectedIndex] as string,
			);
			newDate = new Date();
	...
	public setDate(date: Date): void {
		...
		if (format === "date" || format === "datetime") {
			...
			if (format === "datetime") {
				...
				this.columns[3].selectedIndex =
					this.columns[3].values.indexOf(hourValue);
				this.columns[4].selectedIndex =
					this.columns[4].values.indexOf(minuteValue);
			}
		} else {
			const hourValue = String(date.getHours()).padStart(2, "0");
			const minuteValue = String(date.getMinutes()).padStart(2, "0");
			...
			this.columns[1].selectedIndex =
				this.columns[1].values.indexOf(minuteValue);
		}
